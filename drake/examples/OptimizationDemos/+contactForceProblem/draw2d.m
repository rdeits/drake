function draw2d(platforms, F, x0)
  ...
  for j = 1:length(platforms)
    ...
    for k = 1:2
      quiver(platforms(j).point(1), platforms(j).point(2), cone_edges(1,k), cone_edges(2,k), 0.5, 'color', 'k', 'LineStyle', '--', 'LineWidth', 1, 'ShowArrowHead', 'off', 'AutoScale', 'off');
    end

    quiver(platforms(j).point(1), platforms(j).point(2), F(1,j), F(2,j), 0.1, 'color', 'r', 'LineWidth', 2, 'MaxHeadSize', 0.5, 'AutoScale', 'off')
